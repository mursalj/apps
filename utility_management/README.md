...
Seven suites (`p1`-`p7`) cover service accounts, reading validation, tariff charges,
...
and date. Consumption is `(present - previous) x multiplier`, calculated for you.
...
> properly as `(10^digits - previous + present) x multiplier` and flagged on the bill.
...
queries (`docs/LESSONS_LEARNED.md` Section 41 explains why that matters).
